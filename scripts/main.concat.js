				...
				success: function(json) {
                  ...
                  Mts.formsData.htmlObject();
                        ...
                        html: Mts.formsData.mailHTML
                            ...
                            $('body').append('<a id="urPdf" download="Азбука ремонта" href="' + documentLink + '"></a>');
                            setTimeout(function() {
                              var file = document.getElementById('urPdf');
                              file.click();
                              file.remove();
                            },100);
                        }
		...
		$.ajax({
			...
			success: function(json) {
				...
				$.each(rooms, function(i, article){
					...
					articlesArray.push('<a onclick="_gaq.push(["click", "userRedirect", "articles"]);" href="' + url + '">');
